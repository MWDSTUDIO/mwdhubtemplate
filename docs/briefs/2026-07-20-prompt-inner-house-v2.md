...
4. **Design Studio** — design global + 7 planches par moment, chacune avec sous-planches **Rental** et **Stationery** ; planche papeterie déclinée **Wedding invitations** / **Day-of stationery** ; **palettes : l'équipe saisit 3 à 5 codes hex, l'app génère les pastilles**. **TEMPLATE DE PLANCHE (spécification FINALE validée — le fichier `wedding-board-template.html` de référence accompagne ce prompt, reproduis-le fidèlement)** : format **PAYSAGE éditorial** (~1220px, esprit « concept curaté » type Ophelia) ; fond crème, photo de fond importable très estompée sous voile crème (~13 %) ; **EN-TÊTE CENTRÉ** : eyebrow (nom de planche · événement), **titre = trois mots évocateurs** en Cormorant (« Gathered, Luminous, Unhurried »), puis **paragraphe descriptif du concept** en serif — la voix de la maison, artistique mais limpide, qui transmet une sensation — signé « — Estelle · Madame Wedding Design » (composable par Madame depuis les indications d'Estelle), et dessous **les liens de parenté** (Part of the Global design, Rental details, Stationery details, planche voisine) ; **palette en fine colonne gauche : PETITES pastilles RONDES ~26px + hex éditables, jamais de grands rectangles** ; **collage paysage en quinconce de 7 photos aux RATIOS PRÉ-DÉFINIS** (rangée 1 : portrait 3:4 / paysage 3:2 / deux portraits 3:4 décalés ; rangée 2 : paysage 3:2 / portrait 3:4 central remontant en OVERLAY au-dessus des deux rangées — marge négative (~-96px), ombre portée et z-index supérieur, de sorte qu'il chevauche et « touche » les cartes voisines comme une image posée au milieu du collage / paysage 3:2), object-fit cover, tuiles vides « + », aucune consigne texte ; **rangée de chips « références matières »** éditables (Washed linen · Garden greens · Aged silver…) ; pied : couple/destination/date à gauche, **LOGO MWD en carte NETTE coin bas droit (~104px, ombre légère, PAS de transparence) — signature visible de la maison, protection contre la copie** ; entrée en cascade + tilt 3D discret (~6°), désactivés en prefers-reduced-motion ; contrôles hors de la feuille ; **export PDF PAYSAGE** propre (print CSS puis génération serveur) ; workflow d'approbation (À valider → notification client → approbation/commentaire → notification équipe) ; **export PDF** de chaque planche signée du logo (puppeteer ou react-pdf).
