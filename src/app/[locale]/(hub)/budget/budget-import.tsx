...
import { importBudgetLines, type BudgetImportRow } from "@/app/actions/budget-financial";

/**
 * Budget import (PRD §17) — CSV/XLSX or pasted rows: preview, column
 * mapping proposed and corrected, duplicates against the ledger,
 * review, confirm, an honest report. Vendors are matched against the
 * canonical records, never created here.
 */

const TARGETS = ["ignore", "envelope", "label", "vendor", "budgeted", "committed", "paid", "notes"] as const;
type Target = (typeof TARGETS)[number];

function propose(header: string): Target {
  const h = header.toLowerCase();
  if (/(envelope|categ|poste)/.test(h)) return "envelope";
  if (/(line|ligne|label|libell|description|item|post)/.test(h)) return "label";
  if (/(vendor|prestataire|fournisseur|supplier)/.test(h)) return "vendor";
  if (/(budget|prévu|prevu|forecast|alloc)/.test(h)) return "budgeted";
  if (/(committ|engag|ttc|contract|total)/.test(h)) return "committed";
  if (/(paid|payé|paye|règl|regl|settled)/.test(h)) return "paid";
  return "notes";
}
...
export function BudgetImport({
  ...
  const t = useTranslations("budget.importer");
  ...
  const [review, setReview] = useState<BudgetImportRow[]>([]);
  const [result, setResult] = useState<{ created: number; skipped: number; unmatchedVendors: string[] } | null>(null);

  const known = useMemo(() => new Set(existingLabels.map(norm)), [existingLabels]);
  ...
  const toReview = () => {
    const out: BudgetImportRow[] = rows.map((raw) => {
      const f: Record<string, string> = {};
      const notes: string[] = [];
      raw.forEach((cell, i) => {
        const v = cell.trim();
        if (!v || mapping[i] === "ignore") return;
        if (mapping[i] === "notes") notes.push(v);
        else f[mapping[i]] = v;
      });
      const dup = f.label ? known.has(norm(f.label)) : false;
      return {
        action: f.label && !dup ? "create" : "skip",
        label: f.label ?? "",
        envelope: f.envelope,
        vendor: f.vendor,
        budgeted: toNum(f.budgeted),
        committed: toNum(f.committed),
        paid: toNum(f.paid),
  ...
  const commit = () =>
    startTransition(async () => {
      const r = await importBudgetLines(weddingId, review);
  ...
  return (
    <>
      ...
      <aside aria-label={t("title")} style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(680px, 100%)", background: "var(--white, #fffdf9)", borderLeft: "1px solid var(--line)", padding: "24px 26px", overflow: "auto", zIndex: 50, boxShadow: "-12px 0 40px rgba(34,56,43,.12)" }}>
        ...
        {step === "source" && (
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
              ...
              <a className="btn ghost sm" href="/api/budget-exports?kind=template&format=xlsx" style={{ textDecoration: "none" }}>{t("template")}</a>
              ...
              {review.map((r, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "baseline", flexWrap: "wrap", padding: "7px 12px", borderBottom: "1px solid rgba(201,178,145,.2)", background: r.action === "skip" ? "var(--parchment)" : undefined }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--ink2)" }}>
                    {[r.envelope, r.vendor, r.committed != null ? `TTC ${r.committed}` : null, r.paid != null ? `${t("paidShort")} ${r.paid}` : null].filter(Boolean).join(" · ")}
                  </span>
                  {r.label && known.has(norm(r.label)) && <span style={{ fontSize: 12, color: "var(--bronze)" }}>{t("dupFlag")}</span>}
                </div>
              ))}
            </div>
        ...
        {step === "done" && result && (
          <div style={{ marginTop: 14 }}>
            ...
            {result.unmatchedVendors.length > 0 && (
              <p style={{ fontSize: 13, color: "var(--bronze)" }}>
                {t("unmatchedVendors", { names: result.unmatchedVendors.join(" · ") })}
              </p>
            )}
